...
import {
  Search, Plus, Package, Sparkles, Store as StoreIcon, MapPin,
  ShoppingCart, X, Eye
} from "lucide-react";
...
interface ShopTabProps {
  ...
  getStoreName: (product: any) => string;
  onGoToCart: () => void;
  cartCount: number;
  cartTotal: number;
}
...
export default function ShopTab({
  products, stores, cart, addToCart, getStoreName, onGoToCart, cartCount, cartTotal,
}: ShopTabProps) {
  ...
  return (
    <div className="space-y-6">
      {/* Hero */}
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-emerald-500 via-teal-500 to-cyan-600 p-6 sm:p-10">
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full pl-11 pr-4 py-3 rounded-2xl bg-card border border-border text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-500/50 transition-all"
        />
      </div>
      ...
      <div>
        ...
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 gap-3 text-center rounded-2xl bg-card border border-border/50">
              ...
              {filtered.map((product: any) => {
                ...
                return (
                  <motion.div
                    ...
                    className={cn(
                      "group rounded-2xl overflow-hidden border border-border/50 hover:border-emerald-500/30 transition-all duration-300 flex flex-col cursor-pointer bg-card",
                  ...
                  >
                    <div className="aspect-square bg-gradient-to-br from-muted/50 to-muted flex items-center justify-center relative overflow-hidden">
                      ...
                      {isOutOfStock && (
                        <div className="absolute inset-0 bg-black/40 flex items-center justify-center">
                          <span className="bg-rose-500 text-white px-3 py-1 rounded-lg text-[10px] font-black uppercase">Habis</span>
                        </div>
                      )}
                    </div>
                    <div className="p-3 sm:p-4 flex flex-col flex-1">
                      ...
                      <p className="text-[10px] text-muted-foreground mb-1 truncate">{getStoreName(product)}</p>
                      <p className={cn(
        ...
        {detailProduct && (
          <>
            ...
            <motion.div
              ...
              className="fixed inset-4 sm:inset-auto sm:top-1/2 sm:left-1/2 sm:-translate-x-1/2 sm:-translate-y-1/2 sm:w-full sm:max-w-md z-[90] bg-card rounded-2xl border border-border shadow-2xl flex flex-col max-h-[90vh] overflow-hidden"
            >
              <button
                onClick={() => setDetailProduct(null)}
              ...
              <div className="p-5 sm:p-6 space-y-4 overflow-y-auto">
                ...
                {detailProduct.stock > 0 ? (
                  ...
                  <div className="w-full py-3.5 rounded-xl bg-rose-500/20 text-rose-400 font-bold text-sm text-center">
                    Stok Habis
                  </div>
                )}
              </div>
