...
export default function StoreDetailPage() {
  ...
  const fetchData = () => {
    ...
    Promise.all([
      ...
      fetch(`/api/admin/marketplace/products?storeId=${params.id}`).then((r) =>
        r.json(),
      ),
    ])
    ...
    <div className="p-8 h-full flex flex-col gap-8 overflow-y-auto custom-scrollbar">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="glass-panel p-6 rounded-3xl flex items-center gap-4">
          <div className="w-12 h-12 rounded-xl bg-foreground text-background flex items-center justify-center font-black text-sm">
          ...
          <div>
            ...
            <span
              className={`inline-flex px-2.5 py-1 rounded-md border font-black text-[10px] uppercase tracking-widest mt-1 ${store.status === "active" ? "bg-emerald-500/10 text-emerald-500 border-emerald-500/20" : "bg-rose-500/10 text-rose-500 border-rose-500/20"}`}
            >
              {store.status}
            </span>
          </div>
            ...
            <div className="flex items-center justify-between mb-6">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  Tambahkan produk baru ke toko ini
                </p>
              </div>
              <button
            ...
            <form onSubmit={handleAddProduct} className="space-y-4">
              ...
              <div className="flex justify-end gap-3 pt-4">
                ...
                <button
                  type="submit"
                  disabled={submitting}
                  className="bg-foreground text-background px-5 py-2.5 rounded-xl font-bold text-sm hover:bg-foreground/90 transition-all shadow-lg shadow-foreground/10 disabled:opacity-50"
                >
